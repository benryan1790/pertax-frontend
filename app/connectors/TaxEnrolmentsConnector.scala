...
class TaxEnrolmentsConnector @Inject()(appConfig: ConfigDecorator, http: HttpClient) {
  ...
  def enrolForSa(utr: String, credId: String, groupId: String, action: String)(
    ...
    http
      .POST[SaEnrolment, HttpResponse](s"$serviceUrl$groupId/enrolments/IR-SA~UTR~$utr", saEnrolment)
      .map { response =>
        response.status match {
          case CREATED => true
          case _ =>
            Logger.error(
              s"[TaxEnrolmentsConnector][enrolForSa] failed with status ${response.status}, body: ${response.body}")
            false
        }
      }
      .recover {
        case Upstream4xxResponse(error) =>
          Logger.error(
            s"[TaxEnrolmentsConnector][enrolForSa] Enrolment Store Proxy status ${error.statusCode}, message ${error.message}")
          false
        case exception =>
